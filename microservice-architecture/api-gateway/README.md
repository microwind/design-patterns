...
```mermaid
graph TD
    A[请求到达 Gateway<br/>Request Arrives] --> B[执行 Middleware 链<br/>Execute Middleware Chain]
    B --> C{中间件返回?}
    C -->|返回 Response<br/>Return Response| D[拦截请求<br/>Intercept Request<br/>如 401 Unauthorized]
    C -->|返回 null<br/>Return null| E[通过中间件<br/>Pass Middleware]
    E --> F[前缀匹配路由表<br/>Prefix Match Routes<br/>最长匹配]
    F --> G{匹配成功?}
    G -->|未匹配<br/>No Match| H[返回 404<br/>Return 404]
    G -->|匹配成功<br/>Matched| I[执行 Handler<br/>Execute Handler]
    I --> J[注入 X-Correlation-ID<br/>Inject Correlation ID]
    J --> K[返回 Response<br/>Return Response]
    ...
    style E fill:#69db7c,stroke:#2f9e44,stroke-width:3px,color:#000
    style F fill:#51cf66,stroke:#2b8a3e,stroke-width:3px,color:#fff
    style G fill:#40c057,stroke:#2b8a3e,stroke-width:3px,color:#fff
    style H fill:#ff8787,stroke:#c92a2a,stroke-width:3px,color:#fff
    style I fill:#20c997,stroke:#0ca678,stroke-width:3px,color:#fff
    style J fill:#15aabf,stroke:#0b7285,stroke-width:3px,color:#fff
    style K fill:#0ca678,stroke:#087f5b,stroke-width:3px,color:#fff
```
