...
class OrderProcessor {
  ...
  // 校验职责应拆分到单独的验证类
  private validateId(orderId: number): boolean {
    return orderId % 2 === 0;
  }

  private validateTime(_time: number): boolean {
    return true;
  }

  // 保存数据属于持久化层职责
  private saveOrder(orderId: number): boolean {
    if (orderId % 2 === 0) {
  ...
  private deleteOrder(_orderId: number): boolean {
    return true;
  }
}
...
/**
jarry@jarrys-MBP ts % ts-node SingleResponsibilityBadExample.ts 
oder ID：1001
