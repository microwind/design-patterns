 ...
 */
class Vehicle {
  constructor(public name: string, public type: number, public weight: number) {}
...
class VehicleFactory {
  ...
  private createCar(vehicle: Vehicle): Vehicle {
    console.log(`car has been produced: ${vehicle.type} ${vehicle.name} ${vehicle.weight}`);
    return vehicle;
  }

  private createBus(vehicle: Vehicle): Vehicle {
...
/**
jarry@jarrys-MBP ts % ts-node OpenClosedBadExample.ts 
