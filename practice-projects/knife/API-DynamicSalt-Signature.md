...
在Web应用开发中，API接口的安全性至关重要。随着前后端分离架构的普及，如何确保API调用的合法性、防止数据篡改、抵御重放攻击，成为每个开发者必须面对的问题。
...
1. 前端请求动态盐值生成接口（公开接口，无需密钥）
2. 服务端生成动态盐值并返回
3. 前端携带动态盐值请求签名生成接口
4. 服务端验证动态盐值后，使用密钥生成签名并返回
5. 前端携带签名调用业务接口
...
**简化流程：**
1. 后端直接使用密钥生成签名：`SHA256(appCode + secretKey + apiPath + timestamp)`
2. 后端携带签名调用业务接口
3. 业务接口验证签名
...
1. **密钥隔离**：密钥（secretKey）始终保存在服务端，前端永远无法获取
2. **时效保护**：动态盐值作为"一次性令牌"，有独立的过期时间（如24小时）
3. **接口隔离**：每个接口有独立的固定盐值，动态盐值基于固定盐值生成，确保不同接口的签名互不影响
4. **双重时效控制**：动态盐值有效期（长，如24h） + 签名有效期（短，如10min），形成双层防护
...
**重要说明：** 如果是**后端签名场景**（如微服务调用、定时任务），后端可以安全存储密钥，**无需使用动态盐值**，直接使用密钥生成签名即可（参见 1.2 节）。
...
**校验逻辑：**
1. 使用相同的参数重新计算动态盐值
2. 将计算结果与客户端传来的动态盐值比对
3. 校验时间戳是否在有效期内（通过 `saltTimestamp + ttl` 判断）
...
为确保前后端参数拼接顺序一致，需要对参数进行**ASCII字典序排序**：
...
**实现步骤：**
1. 后端服务读取密钥（从环境变量、配置中心等）
2. 后端直接生成签名：`SHA256(appCode + secretKey + apiPath + timestamp)`
3. 后端携带签名调用业务接口（Header: Sign-appCode, Sign-sign, Sign-time, Sign-path）
4. 业务接口验证签名
...
**实现步骤：**
1. 前端直接调用签名生成接口（不需要先获取动态盐值）
2. 后端生成签名：`SHA256(appCode + secretKey + path + timestamp)`
3. 前端携带签名调用业务接口
...
**实现步骤：**
1. 前端请求动态盐值生成接口
2. 后端生成动态盐值并**存入数据库**（api_dynamic_salt_log表）
3. 前端携带动态盐值请求签名生成接口
4. 后端从数据库查询动态盐值，校验后**标记为已使用**
5. 后端生成含参数签名
6. 前端携带签名和参数调用业务接口
7. 后端提取参数，验证签名（包含参数）
...
│   │   ├── services/sign/
│   │   │   ├── SignService.java             # 签名应用服务
│   │   │   ├── SignValidationService.java   # 签名校验服务
│   │   │   └── DynamicSaltValidationService.java  # 动态盐值校验服务
│   │   └── dto/sign/
│   │       ├── SignDTO.java                 # 签名DTO
│   │       └── DynamicSaltDTO.java          # 动态盐值DTO
│   ├── domain/
│   │   ├── sign/
│   │   │   ├── SignDomainService.java       # 签名领域服务（核心算法）
│   │   │   ├── Sign.java                    # 签名领域对象
...
│   ├── infrastructure/
│   │   └── repository/
│   │       └── SignRepositoryImpl.java      # 签名仓储实现（JDBC）
│   ├── interfaces/
│   │   ├── controllers/sign/
│   │   │   ├── SignController.java          # 签名接口控制器
│   │   │   └── DynamicSaltController.java   # 动态盐值接口控制器
│   │   └── annotation/
...
| 文件 | 作用 |
|------|------|
| `SignDomainService.java` | 实现动态盐值和签名的核心算法 |
| `SignatureUtil.java` | 提供参数排序、多种哈希算法支持 |
| `SignatureInterceptor.java` | 拦截带有 @RequireSign 的接口，进行签名验证 |
| `SignConfig.java` | 集中管理签名相关配置（TTL、算法、模式等） |

### 7.3 数据库脚本

详见 `DynamicSalt.md` 文件中的数据库表结构（支持 MySQL 和 PostgreSQL）。

### 7.4 前端示例代码（JavaScript）
...
```
https://github.com/microwind/design-patterns/tree/main/practice-projects/knife
```
...
本文详细介绍了一套完整的API接口安全方案，通过**动态盐值 + 接口签名**的双重验证机制，有效防止了重放攻击、参数篡改和越权访问。
...
- [国密SM3算法规范](http://www.gmbz.org.cn/main/viewfile/2018011001400692565.html)
- [RFC 6151 - SHA-1安全性分析](https://tools.ietf.org/html/rfc6151)

---

**作者**：Jarry
**日期**：2026-01-04
**联系**：[GitHub Issues](https://github.com/yourusername/knife/issues)
