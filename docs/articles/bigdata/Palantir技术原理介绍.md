...
> 基于 Palantir 官方《[平台概览](https://www.palantir.com/docs/zh/foundry/platform-overview/overview)》、Foundry 文档（Object backend、Action types、AIP features）整理，探讨 Ontology（本体）、数据组织、决策机制与整体技术框架。公司定位、商业模式与案例见同目录下《业务介绍.md》。**ER/OOP 与 Ontology 在存储与行为模式上的对比**见《数据存储与使用方式对比.md》。
...
> **Ontology 旨在表示企业中的决策，而不仅仅是数据。** 它包括：

- 上下文（数据 + 关系）；
- 能力（Action）；
- 约束（权限 / 规则）。

**工程上**：Ontology 是决策运行的载体和契约层；它规定条件、动作与审计，**不代替**人、规则引擎或 LLM 做最终判断。逻辑上是带语义的图（Object / Link），不是平铺表。官方把要素写成「数据—逻辑—操作」：

- **数据（Data）**：事实与状态；形成此决策背景的相关事实或真相是什么？
- **逻辑（Logic）**：规则、模型、外部回调（护栏与分析）；什么组织或业务规则作为此决策的护栏？在不同假设下某些结果的概率是什么？我们在以前的类似情况下做了什么，结果如何？我们的预测和优化模型的输入是什么？
- **操作（Action）**：如何落地、回写、受权限与审计约束。

Ontology结构的组成，就是对象、链接、行为（Action）与约束（含权限/审计）。**易混点**：Action 是独立类型（ActionType），不是实体类上的方法（注意与DDD）。详见《数据存储与使用方式对比.md》第七章。该决策如何在世界上体现？我们如何减少或缩短在 AIP 中做出决策与在生产环境中产生结果之间的步骤？

### 1.2 哲学基础：两种本体观

工程里常对照两种路径：实体本体论（对象为主、关系附属 → ER/OOP）与关系/过程本体论（关系与变化为主 → 图、分布式状态、Palantir）。后者更贴近 Palantir：**除了“有哪些对象”，还要问如何关联、哪些动作能改状态、受何约束。** 因此 Ontology 不是静态目录，而是把**事实、规则、权限与行动**连在一起的语义层。
...
| 基本元素 | 实体、属性、外键关系 | **对象 + 链接 + 属性 + ActionType** <br>对象（ObjectType 实例）映射业务实体；<br>链接（LinkType 实例）具名关系；<br>**ActionType** 描述可执行变更契约，运行时一次调用常称 **Action**，不是实体类上的方法 |
...
Ontology 将数据组织为**对象（Object）**与**链接（Link）**：
...
> 注意：Ontology 不是 图数据库（Neo4j / JanusGraph）。
...
### 2.5 Ontology 在工程上“存在哪里”（避免图数据库误读）
...
- **Object Storage V2 (OSv2)**：新一代 Ontology 数据面；OSv2 在写回上强调 **Action** 路径，并与 Funnel 等组件分工（详见[官方 Object backend 概览](https://palantir.com/docs/foundry/object-backend/overview/)、[OSv1/OSv2 说明](https://palantir.com/docs/foundry/object-databases/object-storage-v1/)）。

下面这张图根据官方文档把后端几个核心服务画在一起，说明它们各自职责，以及流程里面怎么配合。具体调用细节以 Palantir 文档为准；图本身只是方便理解服务边界。
...
官方《平台概览》里平台能力介绍较多，通过整理，可以自上而下拆成 **8 层**。
...
- **Foundry** 是商业客户侧的数据与运营主平台：数据连接、Ontology、模型、应用、分析、大部分 AIP 构建块（AIP Logic、Agent Studio 等按文档在 Foundry 能力集内使用）均落在此；
- **AIP (Artificial Intelligence Platform)** 是**能力剖面/产品集**：在 Ontology 与安全模型之上，提供 LLM 函数、Agent、与运营贯通的 AI 工作流，**不是与 Foundry 并列的另一套“独立数据库”**；
- **Gotham** 面向政府/国防等场景，与 Foundry 产品线区隔，**底层理念（对象、链、安全协作）相近，交付与鉴权环境不同**（本文不展开军品细节）；
- **Apollo** 是跨环境**持续部署与“任务控制”**层，把 Foundry/AIP 的更新以**自主、可编排**的方式推到云、本地、边缘、隔离网（具体能力以合约为准）。

官方将 **Foundry + AIP + Apollo** 合称为**Palantir 平台**在对外叙述中常见；业务侧产品线划分仍以 Gotham / Foundry / AIP 为主。

如果换成更工程化的说法：

- **Foundry** 负责“把数据和应用跑起来”；
- **Ontology** 负责“把业务对象、关系、动作和权限组织起来”；
- **AIP** 负责“把 LLM/Agent 接到这套运行面上”；
- **Apollo** 负责“把整套系统稳定发到不同环境并持续升级”。
...
如果要做同类平台，至少要覆盖下面这些能力层，缺一都会影响落地：

1. **多模式数据接入**
   - 支持批、流、就地访问、零拷贝；
   - 原生覆盖结构化、非结构化、地理空间、时间序列；
   - 支持主流数据湖/仓（Iceberg、Delta、Hudi、Parquet、ES、Kafka、Flink CDC、各类 RDBMS）。
2. **语义层（Ontology 的本地化实现）**
   - 对象—链接—属性—Action 的统一建模；
   - 值类型与约束；
   - 语义搜索、媒体引用等扩展；
   - 自动生成 SDK 和 API。
3. **逻辑层**
   - 模型托管：支持 BYOC、上传预训练模型、平台内训练；
   - 统一 LLM 服务；
   - 外部函数 / Webhook / 自动化工作流；
   - 模型评估与漂移监控。
4. **操作层**
...
6. **安全与治理**
   - 身份、分类、目的多维授权；
   - 全链路加密、审计；
   - 隐私与合规（国标 / GDPR / HIPAA 等按场景适配）。
7. **持续部署**
   - 多环境（云、本地、边缘、隔离网）自动化升级；
   - 灰度、回滚、版本编排。
8. **低代码 + 代码"双轨"开发**
   - 低代码应用与管道构建器；
   - 面向工程师的代码仓库与 IDE 集成；
...
技术能力可以逐步补齐，但组织与商业模式同样关键。这部分见《业务介绍.md》，建议两份文档一起看。

## 六、参考资料

- Palantir 官方[平台概览](https://www.palantir.com/docs/zh/foundry/platform-overview/overview)（中文）
...
- [《Palantir 的"本体论"骗局》](https://zhuanlan.zhihu.com/p/2008601762047746162)（批评视角，补充阅读）
